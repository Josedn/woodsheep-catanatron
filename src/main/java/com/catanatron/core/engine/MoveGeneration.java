...
public final class MoveGeneration {
        ...
        switch (state.currentPrompt) {
            case PLAY_TURN -> {
                ...
                if (get(state, color, "_HAS_ROLLED") == 0) {
                    ...
                    if (get(state, color, "_SETTLEMENTS_AVAILABLE") > 0 && Costs.canAffordSettlement(state, color)) {
                    ...
                    if (canPlayDev(state, color, DevCard.YEAR_OF_PLENTY)) {
                        ...
                        for (int i=0;i<R.length;i++) {
                            for (int j=i;j<R.length;j++) {
                                actions.add(new Action<>(color, ActionType.PLAY_YEAR_OF_PLENTY, new String[]{R[i], R[j]}));
                            }
                        }
                    }
                    if (canPlayDev(state, color, DevCard.ROAD_BUILDING) && !state.board.buildableEdges(color).isEmpty()) {
    ...
    static boolean canPlayDev(State s, PlayerColor c, DevCard card) {
        int idx = s.colors.indexOf(c);
        String base = "P"+idx+"_";
        boolean notPlayedThisTurn = s.playerState.get(base+"HAS_PLAYED_DEVELOPMENT_CARD_IN_TURN") == 0;
