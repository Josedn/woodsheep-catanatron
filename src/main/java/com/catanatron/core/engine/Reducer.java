...
public final class Reducer {
    ...
    private static ActionRecord<?> endTurn(State s, Action<?> a) {
        ...
        for (var card : new DevCard[]{DevCard.KNIGHT, DevCard.YEAR_OF_PLENTY, DevCard.ROAD_BUILDING, DevCard.MONOPOLY}) {
    ...
    private static ActionRecord<?> buyDev(State s, Action<?> a) {
        ...
        s.playerState.put("P"+s.currentPlayerIndex+"_SHEEP_IN_HAND", s.playerState.get("P"+s.currentPlayerIndex+"_SHEEP_IN_HAND") - 1);
        s.playerState.put("P"+s.currentPlayerIndex+"_WHEAT_IN_HAND", s.playerState.get("P"+s.currentPlayerIndex+"_WHEAT_IN_HAND") - 1);
        s.playerState.put("P"+s.currentPlayerIndex+"_ORE_IN_HAND", s.playerState.get("P"+s.currentPlayerIndex+"_ORE_IN_HAND") - 1);
    ...
    private static ActionRecord<?> playYearOfPlenty(State s, Action<?> a) {
        ...
        if (pick.length == 0 || pick.length > 2) throw new IllegalArgumentException("invalid YOP selection");
    ...
    private static ActionRecord<?> playKnight(State s, Action<?> a) {
        ...
        s.playerState.put("P"+idx+"_PLAYED_KNIGHT", s.playerState.get("P"+idx+"_PLAYED_KNIGHT") + 1);
    ...
    private static void maintainLargestArmy(State s, int idxCurrent) {
        ...
        int prevIdx = -1;
        for (int i=0;i<s.colors.size();i++) if (s.playerState.get("P"+i+"_HAS_ARMY") == 1) { prevIdx = i; break; }
        ...
        if (bestCount >= 3 && bestIdx >= 0) {
            ...
            if (prevIdx != bestIdx) {
                if (prevIdx >= 0) { addByIndex(s, prevIdx, "_VICTORY_POINTS", -2); addByIndex(s, prevIdx, "_ACTUAL_VICTORY_POINTS", -2); }
                addByIndex(s, bestIdx, "_VICTORY_POINTS", +2); addByIndex(s, bestIdx, "_ACTUAL_VICTORY_POINTS", +2);
            }
        } else {
            if (prevIdx >= 0) { addByIndex(s, prevIdx, "_VICTORY_POINTS", -2); addByIndex(s, prevIdx, "_ACTUAL_VICTORY_POINTS", -2); }
        }
    ...
    private static ActionRecord<?> playMonopoly(State s, Action<?> a) {
        ...
        s.playerState.put("P"+idx+"_"+resource+"_IN_HAND", s.playerState.get("P"+idx+"_"+resource+"_IN_HAND") + total);
    ...
    private static ActionRecord<?> roll(State s, Action<?> a) {
        ...
        if (sum == 7) {
            ...
            // Payout: for each tile with this number, give 1 per settlement, 2 per city to owner if not robbed
        ...
        return new ActionRecord<>(new Action<>(a.color, a.type, new int[]{d1, d2}), new int[]{d1, d2});
    }
    ...
    private static ActionRecord<?> initialBuildSettlement(State s, Action<?> a) {
        if (s.isInitialBuildPhase) {
            ...
            if (!Costs.canAffordSettlement(s, s.currentColor())) throw new IllegalStateException("cannot afford settlement");
    ...
    private static ActionRecord<?> initialBuildRoad(State s, Action<?> a) {
        if (s.isInitialBuildPhase) {
            ...
            if (buildings < numPlayers) {
                ...
                com.catanatron.core.model.Edge edge = (com.catanatron.core.model.Edge) a.value; int aN = edge.a(); int bN = edge.b();
                ...
                // validate
                if (!isEdgeBuildable(s, s.currentColor(), edge)) throw new IllegalStateException("illegal road placement");
            ...
            if (s.isRoadBuilding && s.freeRoadsAvailable > 0) {
                ...
                if (!Costs.canAffordRoad(s, s.currentColor())) throw new IllegalStateException("cannot afford road");
    ...
    private static ActionRecord<?> buildCity(State s, Action<?> a) {
        int nodeId = (int) a.value;
        if (!Costs.canAffordCity(s, s.currentColor())) throw new IllegalStateException("cannot afford city");
    ...
    private static void maintainLongestRoad(State s) {
        ...
        int prevIdx = -1;
        for (int i=0;i<s.colors.size();i++) {
            if (s.playerState.get("P"+i+"_HAS_ROAD") == 1) { prevIdx = i; break; }
        }
        // Clear flags
    ...
    private static void addResource(State s, PlayerColor c, com.catanatron.core.model.Resource r, int amount) {
        int idx = s.colors.indexOf(c);
    ...
    private static boolean isEdgeBuildable(State s, PlayerColor color, com.catanatron.core.model.Edge edge) {
        long key = (((long)Math.min(edge.a(), edge.b()))<<32) | (((int)Math.max(edge.a(), edge.b())) & 0xffffffffL);
        for (var e : s.board.buildableEdges(color)) {
            long k2 = (((long)Math.min(e.a(), e.b()))<<32) | (((int)Math.max(e.a(), e.b())) & 0xffffffffL);
